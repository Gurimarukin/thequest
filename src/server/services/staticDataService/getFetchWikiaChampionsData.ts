...
export const getFetchWikiaChampionsData = (
  ...
  httpClient: HttpClient,
): Future<List<WikiaChampionData>> => pipe(
      httpClient.text([championDataUrl, 'get']),
      Future.chainEitherK(DomHandler.of),
      Future.chainEitherK(domHandler => pipe(
      ...
      Future.map(flow(
        DictUtils.entries,
        List.partitionMap(([englishName, rawChampion]) => pipe(
           RawWikiaChampionData.decoder.decode(rawChampion),
          Either.bimap(decodeErrorString(`RawWikiaChampionData (${JSON.stringify(englishName)})`)(rawChampion), WikiaChampionData.fromRaw(englishName)),
        )),
      )),
      Future.chainFirstIOEitherK(({ left: errors }) => List.isNonEmpty(errors) ? logger.warn(pipe(
        errors,
        List.mkString('\n', '\n', ''),
      )) : IO.notUsed),
