...
type RiotCurrentLolGameInfo = Omit<RawCurrentGameInfo, 'bannedChampions' | 'participants'> & {
  isDraft: boolean
...
const decoder = pipe(
  rawDecoder,
  D.map(({ bannedChampions, participants, ...game }): RiotCurrentLolGameInfo => {
...
const RiotCurrentLolGameInfo = { decoder }

export { RiotCurrentLolGameInfo }
